...
for nu_val in nu_vals
    trixi_include("../examples/elixir_shallowwater_moments_smooth_wave.jl",
                  equations = ShallowWaterLinearizedMomentEquations1D(gravity = 9.81, H0 = 0.0, n_moments = 2, nu = nu_val),
...
with_theme(theme_latexfonts()) do
    ...
    ax_ns = Axis(g_ns[1,1], xlabel = L"t", ylabel = L"\mathcal{D}_{Ns}", xlabelsize = sizelblx, ylabelsize = sizelbly)
    ax_mm = Axis(g_mm[1,1], xlabel = L"t", ylabel = L"\mathcal{D}_{MM}", xlabelsize = sizelblx, ylabelsize = sizelbly)

    # Plot data for polynomial degrees
    for (i, nu_val) in enumerate(nu_vals)
        lines!(ax_ns, df_ns[i].time, df_ns[i].var"dwdP_Ns", color=colors[i], linewidth = 2)
        lines!(ax_mm, df_mm[i].time, df_mm[i].var"dwdP_MM", color=colors[i], linewidth = 2)
    end

    # Plot reference line at zero
    lines!(ax_ns, df_ns[1].time, 0.0*df_ns[1].var"dwdP_Ns", color = :black, linestyle = :dash, linewidth = 1)
    lines!(ax_mm, df_mm[1].time, 0.0*df_mm[1].var"dwdP_MM", color = :black, linestyle = :dash, linewidth = 1)
    ...
    legend = Legend(f[2,1:2], 
        [[LineElement(linestyle = :solid, color = colors[i], linewidth = 2) for i in 1:length(nu_vals)]...],
