...
trixi_include("../examples/elixir_shallowwater_moments_stone_throw.jl",
               equations = ShallowWaterMomentEquations1D(gravity = 9.812, H0 = 1.75, n_moments = 2),
...
with_theme(theme_latexfonts()) do
    ...
    ax_h_wb = Axis(g_h[1,1], xlabel = L"x", ylabel = L"h")
    ax_v_wb = Axis(g_v[1,1], xlabel = L"x", ylabel = L"u_m")
    ax_a1_wb = Axis(g_a1[1,1], xlabel = L"x", ylabel = L"\alpha_1")
    ax_a2_wb = Axis(g_a2[1,1], xlabel = L"x", ylabel = L"\alpha_2")

    
    wb_color = Makie.wong_colors()[1]
    nwb_color = Makie.wong_colors()[2]

    label_wb = lines!(ax_h_wb, pd_wb.x, pd_wb.data[:,1], color = wb_color)
    label_b  = lines!(ax_h_wb, pd_wb.x, pd_wb.data[:,end], color=:black, linestyle = :dash)
    lines!(ax_v_wb, pd_wb.x, pd_wb.data[:,2], color = wb_color)
    lines!(ax_a1_wb, pd_wb.x, pd_wb.data[:,3], color = wb_color)
    lines!(ax_a2_wb, pd_wb.x, pd_wb.data[:,4], color = wb_color)

    label_nwb = lines!(ax_h_wb, pd_wb.x, pd_nwb.data[:,1], color = nwb_color)
    lines!(ax_v_wb, pd_wb.x, pd_nwb.data[:,2], color = nwb_color)
    lines!(ax_a1_wb, pd_wb.x, pd_nwb.data[:,3], color = nwb_color)
    lines!(ax_a2_wb, pd_wb.x, pd_nwb.data[:,4], color = nwb_color)
...
with_theme(theme_latexfonts()) do
    ...
    ax_left = Axis(f[1, 1], xlabel = "Time", ylabel = L"\frac{1}{|\Omega|}\int_{\Omega} \mathbb{E}(t) \,\text{d}x", xlabelsize = sizelblx, ylabelsize = sizelbly)
    ax_right = Axis(f[1, 2], xlabel = "Time", ylabel = L"\frac{1}{|\Omega|}\int_{\Omega} |h(t) - h(0)| \,\text{d}x", xlabelsize = sizelblx, ylabelsize = sizelbly)
